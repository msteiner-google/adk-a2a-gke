...
**Peers are attached as tools, not sub-agents.** :func:`build_agent` asks the
injected ``AgentResolver`` for whatever peers the cluster configuration resolved
for this process, and appends them to the agent's own tools. Each arrives as a
``PeerTool`` carrying that peer's typed payload contract, so delegating means
composing an explicit request rather than forwarding the conversation — see
``app/cluster/peer_tool.py`` and ``docs/design-decisions.md``. A
leaf agent resolves to no peers and simply gets its own tools.
"""
...
class AgentSpec:
    ...
    Attributes:
        ...
        peers: Default peer names this agent can delegate to over A2A. Resolved
            to in-cluster URLs by ``app/cluster/config.py``, wrapped as typed
            ``PeerTool``s by ``app/cluster/resolver.py``, and overridable via the
            ``A2A_PEERS`` environment variable. Empty for leaf agents.
    """
...
def build_agent(spec: AgentSpec, models: Models, resolver: AgentResolver) -> LlmAgent:
    ...
    Args:
        ...
        resolver: The resolver that turns configured peers into typed tools.
    ...
    return LlmAgent(
        ...
        # Uniform for every agent: its own tools, then whatever peers the
        # cluster config resolved for this process (empty for a leaf agent).
